...
class Step:
    ...
    def validate(self, data):
        ...
        if (data):
            if not(data.get("hands") == (-1,-1)):
                return True
        """
        if (self.index == 1):
            #holding pedal wrench
                return True
        elif (self.index == 2):
                #pedal wrench intersects pedal
                return True
        elif (self.index == 3):
            if data % 10:
                #pedal wrench rotations
                return False
        elif (self.index == 4):
                #pedal wrench intersection decreasing/off screen for logner than 5 sec?
                return True
        """
        return True